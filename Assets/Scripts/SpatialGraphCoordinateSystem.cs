...
namespace QRTracking
{
    public class SpatialGraphCoordinateSystem : MonoBehaviour
    {
        ...
        private float physicalSideLength = 0.0f;

        public void setSideLength(float sideLength)
        {
            physicalSideLength = sideLength;
        }
        ...
        protected void MovePoseToCenter(Pose pose, float physicalSideLength)
        {
            ...
            pose.position += (pose.rotation * (deltaToCenter * Vector3.right) + pose.rotation * (deltaToCenter * Vector3.up));
            //pose.position += new Vector3(deltaToCenter, -deltaToCenter, 0.0f);
            gameObject.transform.SetPositionAndRotation(pose.position, pose.rotation);
        }
